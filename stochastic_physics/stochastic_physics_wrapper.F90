  ...
  subroutine stochastic_physics_wrapper (IPD_Control, IPD_Data, Atm_block, nthreads, ierr)
    ...
    initalize_stochastic_physics: if (.not. is_initialized) then
      ...
      if ( IPD_Control%stochastic%lndp_type == 1 ) then ! this scheme sets perts once
         ...
         do nb=1,nblks
            IPD_Data(nb)%Stochastic%sfc_wts(:,:) = sfc_wts(nb,1:IPD_Control%blksz(nb),:)
         end do
         deallocate(sfc_wts)
      end if
      ...
      if (IPD_Control%stochastic%do_sppt .OR. IPD_Control%stochastic%do_shum .OR. IPD_Control%stochastic%do_skeb .OR. &
         ...
         if (IPD_Control%stochastic%do_sppt) then
            do nb=1,nblks
                IPD_Data(nb)%Stochastic%sppt_wts(:,:) = sppt_wts(nb,1:IPD_Control%blksz(nb),:)
            end do
         end if
         if (IPD_Control%stochastic%do_shum) then
            do nb=1,nblks
                IPD_Data(nb)%Stochastic%shum_wts(:,:) = shum_wts(nb,1:IPD_Control%blksz(nb),:)
            end do
         end if
         if (IPD_Control%stochastic%do_skeb) then
            do nb=1,nblks
                IPD_Data(nb)%Stochastic%skebu_wts(:,:) = skebu_wts(nb,1:IPD_Control%blksz(nb),:)
                IPD_Data(nb)%Stochastic%skebv_wts(:,:) = skebv_wts(nb,1:IPD_Control%blksz(nb),:)
            end do
         end if
         if (IPD_Control%stochastic%do_spp) then
            do n=1,IPD_Control%stochastic%n_var_spp
               ...
               case('pbl')
                 do nb=1,Atm_block%nblks
                     IPD_Data(nb)%Stochastic%spp_wts_pbl(:,:) = spp_wts(nb,1:IPD_Control%blksz(nb),:,n)
                 end do
               case('sfc')
                 do nb=1,Atm_block%nblks
                     IPD_Data(nb)%Stochastic%spp_wts_sfc(:,:) = spp_wts(nb,1:IPD_Control%blksz(nb),:,n)
                 end do
               case('mp')
                 do nb=1,Atm_block%nblks
                     IPD_Data(nb)%Stochastic%spp_wts_mp(:,:) = spp_wts(nb,1:IPD_Control%blksz(nb),:,n)
                 end do
               case('gwd')
                 do nb=1,Atm_block%nblks
                     IPD_Data(nb)%Stochastic%spp_wts_gwd(:,:) = spp_wts(nb,1:IPD_Control%blksz(nb),:,n)
                 end do
               case('rad')
                 do nb=1,Atm_block%nblks
                     IPD_Data(nb)%Stochastic%spp_wts_rad(:,:) = spp_wts(nb,1:IPD_Control%blksz(nb),:,n)
                 end do
               case('cu_deep')
                 do nb=1,Atm_block%nblks
                     IPD_Data(nb)%Stochastic%spp_wts_cu_deep(:,:) = spp_wts(nb,1:IPD_Control%blksz(nb),:,n)
                 end do
               end select
            end do
         end if
      ...
      if (IPD_Control%stochastic%do_ca) then
       ...
       if(IPD_Control%stochastic%ca_sgs)then
         do nb=1,nblks
             ...
             condition  (nb,1:IPD_Control%blksz(nb)) = IPD_Data(nb)%Stochastic%condition(:)
             ca_deep_cpl(nb,1:IPD_Control%blksz(nb)) = IPD_Data(nb)%Stochastic%ca_deep(:)
             ca_turb_cpl(nb,1:IPD_Control%blksz(nb)) = IPD_Data(nb)%Stochastic%ca_turb(:)
             ca_shal_cpl(nb,1:IPD_Control%blksz(nb)) = IPD_Data(nb)%Stochastic%ca_shal(:)
         enddo
         ...
         do nb=1,nblks
             IPD_Data(nb)%Stochastic%ca_deep(:) = ca_deep_cpl (nb,1:IPD_Control%blksz(nb))
             IPD_Data(nb)%Stochastic%ca_turb(:) = ca_turb_cpl (nb,1:IPD_Control%blksz(nb))
             IPD_Data(nb)%Stochastic%ca_shal(:) = ca_shal_cpl (nb,1:IPD_Control%blksz(nb))
         enddo
       endif
       if(IPD_Control%stochastic%ca_global)then
          ...
          ! Copy contiguous data back
          do nb=1,nblks
             IPD_Data(nb)%Stochastic%ca1(:) = ca1_cpl(nb,1:IPD_Control%blksz(nb))
             IPD_Data(nb)%Stochastic%ca2(:) = ca2_cpl(nb,1:IPD_Control%blksz(nb))
             IPD_Data(nb)%Stochastic%ca3(:) = ca3_cpl(nb,1:IPD_Control%blksz(nb))
          enddo
       endif
